...
void scroll_screen(void) {
	...
	for(y=0;y<(VGA_SCREEN_HEIGHT - 2);y++){
	...
	for(x=0,y=(VGA_SCREEN_HEIGHT - 2);x<(VGA_SCREEN_WIDTH-8);x++){
			pos = y * VGA_SCREEN_WIDTH + x; //计算16位pos值
			pos_1 = (y + 1) *VGA_SCREEN_WIDTH + x;
			videobuf[pos*2 + 0] = videobuf[pos_1*2 + 0]; //每个字符向上移动一行
			videobuf[pos*2 + 1] = videobuf[pos_1*2 + 1]; //每个字符颜色向上移动一行
	...
	for(x=(VGA_SCREEN_WIDTH-8),y=(VGA_SCREEN_HEIGHT - 3);x<VGA_SCREEN_WIDTH;x++){
			pos = y * VGA_SCREEN_WIDTH + x; //计算16位pos值
			videobuf[pos*2 + 0] = ' '; //对于时钟那一栏来说，将他们设置为黑色的空格，也就是说不滚动wallclock
			videobuf[pos*2 + 1] = 0x7;
		}
